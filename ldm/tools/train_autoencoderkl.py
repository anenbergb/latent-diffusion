...
# Modifications Copyright 2025 Bryan Anenberg
#
...
from ldm.data import CORRUPTED_IMAGENET_IMAGES
import random
...
def sample_imagenet_validation_images(num_images=16):
    dataset = load_dataset("ILSVRC/imagenet-1k", split="validation", trust_remote_code=True)
    valid_ids = [ i for i in range(len(dataset)) if i not in CORRUPTED_IMAGENET_IMAGES["validation"]]
    sample_ids = random.sample(valid_ids, num_images)
    dataset = dataset.select(sample_ids)
    return dataset

@torch.no_grad()
def log_validation(vae, args, accelerator, weight_dtype, step, is_final_validation=False, validation_dataset=None):
    ...
    if validation_dataset is None:
        pil_images = [Image.open(path) for path in args.validation_image]
    else:
        pil_images = [validation_dataset[i]["image"] for i in range(len(validation_dataset))]
    

    for validation_image in pil_images:
        targets = image_transforms(validation_image.convert("RGB")).to(accelerator.device, weight_dtype)
        ...
        images.append(targets.cpu().squeeze(0))
        images.append(reconstructions.cpu().squeeze(0))

    image_grid = torchvision.utils.make_grid(images,nrow=2, padding=0)

    tracker_key = "test" if is_final_validation else "validation"
    for tracker in accelerator.trackers:
        if tracker.name == "tensorboard":
            tracker.writer.add_image(f"{tracker_key}: Original (left), Reconstruction (right)", np.asarray(image_grid), step)
        elif tracker.name == "wandb":
            tracker.log(
                {
                    f"{tracker_key}: Original (left), Reconstruction (right)": wandb.Image(image_grid)
...
def make_train_dataset(args, accelerator):
    ...
    if args.dataset_name == "ILSVRC/imagenet-1k":
        ...
        for split, corrupt_ids in CORRUPTED_IMAGENET_IMAGES.items():
            if split in dataset:
                # Remove corrupted images from the dataset
                dataset[split] = dataset[split].select(
                    [i for i in range(len(dataset[split])) if i not in CORRUPTED_IMAGENET_IMAGES[split]]
                )
                logger.warning(
                    f"Removed {len(corrupt_ids)} corrupted images from the {split} split of the ILSVRC/imagenet-1k dataset."
                )
...
def main(args):
    ...
    if args.validation_image is None:
        val_dataset = sample_imagenet_validation_images(num_images=16)
